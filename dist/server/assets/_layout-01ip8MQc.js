...
import { useRouter, useLocation, useNavigate, Outlet } from "@tanstack/react-router";
import * as React from "react";
import { useState, useEffect } from "react";
import { useQueryClient, useMutation, useQuery } from "@tanstack/react-query";
import { toast } from "sonner";
import { ChevronsUpDown, X, User, Mail, Phone, PlusIcon, LogOut } from "lucide-react";
import Cookies from "js-cookie";
import * as PopoverPrimitive from "@radix-ui/react-popover";
import { c as cn, B as Button } from "./button-Cul2gyf-.js";
import { S as SalesTableLogo } from "./Salestablelogo-Cl8l0GJc.js";
import { C as CreateWorkspaceAPI, g as getAllUsersAPI, r as ragStats, a as ragSync } from "./ragServices-BTI_KB5q.js";
    ...
    /* @__PURE__ */ jsx(
      PopoverContent,
      {
        ...
        children: /* @__PURE__ */ jsxs("div", { className: "flex flex-col", children: [
          ...
          /* @__PURE__ */ jsx("ul", { className: "max-h-36 overflow-y-auto ", children: filteredWorkspaces?.length ? filteredWorkspaces.map((workSpaceObj) => /* @__PURE__ */ jsx(
            "li",
            {
              onClick: () => handleSelectWorkspace(workSpaceObj),
              className: `px-4 py-1.5 text-sm hover:bg-gray-100 cursor-pointer ${!workSpaceObj.is_active ? "text-gray-400 cursor-not-allowed" : selectedWorkspace?.id === workSpaceObj.id ? "bg-primary-200/40" : ""}`,
...
function MainLayout() {
  ...
  const { data, isPending, isError } = useQuery({
    ...
    queryFn: async () => {
      if (user.user_type === "ADMIN" && selectedWorkspace?.id === void 0) {
        const res = await ragStats(`/files/stats`);
        if (res?.status === 200 || res?.status === 201) return res.data;
      } else if (selectedWorkspace?.user_type === "CUSTOMER" && user.user_type === "ADMIN" && selectedWorkspace?.id) {
  ...
  const ragSyncFiles = useMutation({
    ...
    onError: (error) => toast.error(error?.message || "failed to sync")
  });
  const handleRagSync = () => {
        ...
        /* @__PURE__ */ jsx("div", { className: "flex justify-end items-center ml-2 mr-2", children: /* @__PURE__ */ jsxs("div", { className: "flex gap-2 justify-end", children: [
          /* @__PURE__ */ jsxs("div", { className: "flex justify-center gap-[10px] items-center bg-white w-[120px] h-10 rounded-lg shadow-sm px-3 md:px-4", children: [
            /* @__PURE__ */ jsxs("div", { className: "flex gap-[10px] justify-center items-center", children: [
              ...
              /* @__PURE__ */ jsx("p", { className: "text-normal font-semibold text-gray-900", children: isPending ? "..." : data?.data?.files })
            ] }),
          ...
          /* @__PURE__ */ jsxs("div", { className: "flex justify-center gap-[10px] items-center bg-white w-[120px] h-10 rounded-lg shadow-sm px-3 md:px-4", children: [
            /* @__PURE__ */ jsxs("div", { className: "flex gap-[10px] justify-center items-center", children: [
              ...
              /* @__PURE__ */ jsx("p", { className: "text-normal font-semibold text-gray-900", children: isPending ? "..." : data?.data?.urls })
            ] }),
          ...
          /* @__PURE__ */ jsxs("div", { className: "flex justify-center gap-[10px] items-center bg-white w-[160px] md:w-[180px] h-10 rounded-lg shadow-sm px-3 md:px-4", children: [
            /* @__PURE__ */ jsxs("div", { className: "flex gap-[10px] justify-center items-center", children: [
              ...
              /* @__PURE__ */ jsx("p", { className: "text-normal font-semibold text-gray-900", children: isPending ? "..." : data?.data?.ready })
            ] }),
          ...
          /* @__PURE__ */ jsxs("div", { className: "flex justify-center gap-[10px] items-center bg-white w-[120px] h-10 rounded-lg shadow-sm px-3 md:px-4", children: [
            /* @__PURE__ */ jsxs("div", { className: "flex gap-[10px] justify-center items-center", children: [
              ...
              /* @__PURE__ */ jsx("p", { className: "text-normal font-semibold text-gray-900", children: isPending ? "..." : data?.data?.failed })
            ] }),
